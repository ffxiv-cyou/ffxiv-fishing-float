...
<div class="p-4 max-w-3xl mx-auto" bind:this={rootNode}>
  ...
  <Heading tag="h2" class="text-xl mt-4">竿时</Heading>
  <p>竿时指从抛竿到咬钩的时间。</p>
  <p>
    为保证统计口径统一，在本悬浮窗中，抛竿时间和咬钩时间均为服务器回包时间。
  </p>
  <p>在统计中，我们会按照不同鱼饵、是否使用撒饵技能对数据进行分类。</p>
  <p>我们会忽略在使用拟饵技能后5.3秒内的竿时数据。</p>
  <Heading tag="h2" class="text-xl mt-4">竿时区间</Heading>
  <p>我们使用平均分布模型来对一条鱼的竿时做建模。</p>
  <p>对于鱼的最大竿时和最小竿时，我们记为“全部区间”。</p>
  ...
  <p>
    若用户选择不提钩，我们则会根据竿时和现有鱼的概率信息，使用最大期望法推断鱼的咬钩概率。
  </p>
  <Heading tag="h2" class="text-xl mt-4">脱钩率</Heading>
  <p>当用户提钩后，若鱼脱钩，我们会统计不同竿型的脱钩率。</p>
  <p>统计脱钩率时，会忽略掉因获得力不够导致的脱钩情况。</p>
  <p>我们还会尝试根据竿种和竿时，使用最大期望法推断鱼的脱钩率。</p>
  <p>
    当用户使用耐心/耐心II技能时，我们会根据技能对脱钩率的影响调整脱钩率的计算。
  </p>
  <p>
    脱钩率的置信度代表着此脱钩率的可靠程度。当置信度为1时，表示该脱钩率非常可靠。通常出现在当前鱼的竿型只有这条鱼的情况下。
  </p>
  <Heading tag="h1" class="text-2xl mt-4">系统建模</Heading>
  <p>我们根据已有的样本数据，确定了一条鱼的竿时分布服从平均分布。</p>
  <p>考虑到钓鱼时有如下的特殊行为：</p>
  <List tag="ul">
    <Li
      >能否钓到指定的鱼完全由抛竿时间决定。就算在鱼的窗口时间最后一秒抛竿，也有可能钓到对应的鱼。</Li
    >
    <Li
  ...
  <List tag="ol">
    <Li
      >抛竿时（或使用雄心/谦逊技能时），首先使用内置的概率表，根据当前技能的状态，随机选择一条鱼。</Li
    >
    <Li>根据这条鱼的竿时区间，随机生成一个竿时，在指定时间发出咬钩信号。</Li>
    <Li>提钩时，根据这条鱼的脱钩率，决定是否脱钩。</Li>
  </List>
  <Heading tag="h2" class="text-xl mt-4">竿时反推概率</Heading>
  <p>
  ...
  <p>根据贝叶斯定理，当竿时为 \(t\) 时，对应类别 \(F_i\) 的概率为：</p>
  <p>
